...
function ajouterSuivi($mysqli, $data) {
    ...
    $query = "INSERT INTO suivi_courrier 
...
function supprimerSuivi($mysqli, $id_suivi) {
    $query = "DELETE FROM suivi_courrier WHERE id_suivi = ?";
    $stmt = $mysqli->prepare($query);
...
function getCourriersForSelect($mysqli) {
    $query = "SELECT id_courrier, Numero_Courrier, Objet FROM courrier ORDER BY date DESC";
...
function getCountTotalSuiviCourrier($mysqli) {
    $query = "SELECT COUNT(*) as total FROM suivi_courrier";
    $result = $mysqli->query($query);
