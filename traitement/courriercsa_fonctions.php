...
function enregistrerCourrier($connexion, $data, &$errors = []) {
    ...
    if (empty($data['Numero_Courrier'])) {
        ...
        if (!preg_match('/^COUR-\d{4}-\d{3}$/', $numero)) {
            ...
            $checkQuery = "SELECT id_courrier FROM courriercsa WHERE Numero_Courrier = '$numero'";
    ...
    $query = "INSERT INTO courriercsa (Numero_Courrier, date, Objet, pdf, Nature, Type, Expediteur) 
...
function getListeCourrier($connexion, $limit = 1000) {
    $limit = (int)$limit;
    $query = "SELECT * FROM courriercsa ORDER BY id_courrier DESC LIMIT $limit";
...
function getCourrierById($connexion, $id) {
    $id = (int)$id;
    $query = "SELECT * FROM courriercsa WHERE id_courrier = $id";
    $result = mysqli_query($connexion, $query);
...
function supprimerCourrier($connexion, $id) {
    ...
    $id = (int)$id;
    $query = "DELETE FROM courriercsa WHERE id_courrier = $id";
...
function getFilteredCourrier($connexion, $filters = [], $page = 1, $itemsPerPage = 10) {
    $query = "SELECT * FROM courriercsa WHERE 1=1";
    $params = [];
...
function countFilteredCourriers($connexion, $filters = []) {
    $query = "SELECT COUNT(*) as total FROM courriercsa WHERE 1=1";
